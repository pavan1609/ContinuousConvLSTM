...
from models.continuous_conv import ContinuousConv2d
...
class DeepConvLSTM(nn.Module):
    ...
    def __init__(
        ...
        conv_shared_mlp_across_blocks: bool = False,
        ...
        self.conv_shared_mlp_across_blocks = bool(conv_shared_mlp_across_blocks)
        ...
        if self.conv_type == "standard":
            ...
            self.shared_block_mlps = nn.ModuleDict()
            ...
            for fs in self.supported_sample_rates:
                ...
                shared_block_mlp = None
                if self.conv_shared_mlp_across_blocks:
                    shared_block_mlp = nn.Sequential(
                        nn.Linear(1, fs_hidden),
                        nn.ReLU(inplace=True),
                        nn.Linear(fs_hidden, fs_rank),
                    )
                    self.shared_block_mlps[str(int(fs))] = shared_block_mlp
